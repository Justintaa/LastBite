...
**Live demo:** https://lastbite-4tyb.onrender.com
*(Free-tier hosting — the app may take ~30–50 seconds to wake up on first load if it's been idle.)*
...
- Deployed to production on Render, with Gunicorn, Whitenoise static file serving, and environment-based secrets
...
**Payments:** Stripe API
**DevOps/Tools:** Docker, Docker Compose, Git, Render, Gunicorn, Whitenoise
...
```
git clone https://github.com/Justintaa/LastBite.git
cd LastBite
```
...
| `POSTGRES_HOST`                | PostgreSQL host (`db` for local Docker Compose)       |
| `POSTGRES_PORT`                | PostgreSQL port (defaults to `5432`)                  |
| `DJANGO_DEBUG`                 | Django debug setting for local development            |
| `DJANGO_SECRET_KEY`            | Django secret key                                     |
| `DJANGO_ALLOWED_HOSTS`         | Comma-separated list of allowed hosts                 |
| `DJANGO_CSRF_TRUSTED_ORIGINS`  | Comma-separated list of trusted origins (e.g. your live URL, with scheme) |
| `DJANGO_ENV`                   | Set to `production` to run Gunicorn instead of the dev server |
| `STRIPE_PUBLISHABLE_KEY`       | Stripe publishable key                                |
| `STRIPE_SECRET_KEY`            | Stripe secret key                                     |

## What I Worked On

As part of the team, I contributed to the design and development of the application's full-stack functionality, including Django views, templates, database-backed features, user-facing pages, and workflow improvements across the marketplace experience. I also handled the production deployment: containerizing the app for Render, configuring Whitenoise/Gunicorn, and setting up managed PostgreSQL.
...
This project was created for academic purposes and is intended as a portfolio demonstration of full-stack Django development. API keys, database passwords, and secret keys should be stored locally in `.env` (or your host's environment variable settings) and should not be committed to GitHub. The live demo uses Stripe in test mode where applicable.
